...
class SecurityLogDemonstrationTest {
    ...
    void securityFix_TokensAreMaskedToPreventSensitiveDataExposureInLogs() {
        ...
        String googleOAuth2Token =
                "ya29.a0AfH6SMC_HIGHLY_SENSITIVE_DATA_1234567890abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";
        String jwtToken =
        ...
        assertNotEquals(googleOAuth2Token, maskedOAuth2, "CRITICAL: Full OAuth2 token must never equal masked version");
        assertNotEquals(jwtToken, maskedJWT, "CRITICAL: Full JWT token must never equal masked version");
        assertNotEquals(apiKeyToken, maskedAPIKey, "CRITICAL: Full API key must never equal masked version");
        assertNotEquals(bearerToken, maskedBearer, "CRITICAL: Full Bearer token must never equal masked version");

        // 2. SENSITIVE MIDDLE PARTS are completely removed
        assertFalse(
                maskedOAuth2.contains("HIGHLY_SENSITIVE_DATA"),
                "CRITICAL: Sensitive data must not appear in masked token");
        assertFalse(
                maskedJWT.contains("eyJzdWIiOiIxMjM0NTY3ODkw"),
                "CRITICAL: JWT payload must not appear in masked token");
        assertFalse(
                maskedAPIKey.contains("EXTREMELY_SENSITIVE_API_KEY"),
                "CRITICAL: API key sensitive part must not appear in masked token");

        // 3. MASKING PATTERN is consistently applied
        assertEquals(
                "ya29****WXYZ", maskedOAuth2, "OAuth2 token should be masked showing only first 4 and last 4 chars");
        assertEquals("eyJh****sw5c", maskedJWT, "JWT token should be masked showing only first 4 and last 4 chars");
        assertEquals("sk-E****wxyz", maskedAPIKey, "API key should be masked showing only first 4 and last 4 chars");
        assertEquals(
                "Bearer ya29****WXYZ", maskedBearer, "Bearer token should preserve prefix and mask the token part");

        // 4. ENOUGH DEBUGGING INFO remains for troubleshooting
        assertTrue(maskedOAuth2.startsWith("ya29"), "Masked token should preserve recognizable prefix for debugging");
        assertTrue(maskedJWT.startsWith("eyJh"), "Masked JWT should preserve header prefix for debugging");
        assertTrue(maskedAPIKey.startsWith("sk-"), "Masked API key should preserve type prefix for debugging");

        // 5. SECURE AUTHENTICATION MESSAGES can be safely logged
        String secureMessage =
                SecurityLogUtil.createSecureAuthMessage("OAuth2", "user@example.com", googleOAuth2Token, true);

        assertTrue(secureMessage.contains("ya29****WXYZ"), "Secure auth message should contain masked token");
        assertFalse(
                secureMessage.contains("HIGHLY_SENSITIVE_DATA"),
